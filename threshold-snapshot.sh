...
make_opts() {
    ...
    local _OPTS=$(getopt -o c:s:n:d:t:v:u:h --long chain:,service_file:,daemon_name:,daemon_dir:,threshold:,volume:,user:,help \
...
run_snapshot() {
  if [[ $(df -h | grep $VOLUME | awk '{print $5}' | cut -d'%' -f 1) -gt $THRESHOLD ]]; then
    ...
    echo "stopping blockchain daemon and performing unsafe-reset-all // snapshot restore"
